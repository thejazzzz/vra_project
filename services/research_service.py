...
async def process_research_task(query: str) -> Dict:
    ...
    # Store in DB — commit per paper to avoid batch failure
    for paper, fulltext in zip(papers, pdf_texts):
        ...
        # ensure normalized id reflected back into API output
        paper["id"] = paper_id

        # clean metadata fields as well, just in case
        ...
        try:
            ...
            if existing:
                ...
                existing.abstract = (
                    paper["summary"] if is_nonempty_text(paper["summary"]) else existing.abstract
                )
